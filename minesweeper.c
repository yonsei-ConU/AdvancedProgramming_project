...
void init() {
    ...
    printf("@ 칸은 아직 열리지 않은 칸입니다. 빈 검은색 칸은 숫자가 0인 칸을 나타냅니다.\n\n");
    printf("좌표 앞에 -를 한 번 이상 붙일 경우 (ex. -4 7 또는 -9 -2) 깃발을 설치하는 것으로 간주됩니다.\n");
    printf("깃발이 설치된 칸은 ! 기호가 보여집니다. 깃발이 설치된 자리에 깃발을 다시 설치하면 깃발이 없어집니다.\n\n");
    printf(sample_1);
    printf("이제 게임을 시작하겠습니다.\n\n");

    printf("지뢰판의 가로 크기와 세로 크기를 입력해 주세요 (5~20)\n가로와 세로는 공백으로 구분해 주세요 : ");
    while(1) {
        scanf("%d %d", &n, &m);
        if((5 <= n && n <= 20) && (5 <= m && m <= 20)) break;
        else printf("범위에 맞춰 다시 입력해 주세요 : ");
    }

    while(1) {
        printf("\n추천 난이도 : 쉬움(%d) 보통(%d) 어려움(%d)\n",
            ((n * m - 9) / 10), ((n * m - 9) / 6), ((n * m - 9) / 4));
        printf("참고 : 어려움 이상으로 개수를 설정할 경우 실력만으로 게임을 클리어하기가 매우 어려워집니다.\n\n");
